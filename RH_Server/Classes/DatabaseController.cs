...
namespace RH_Server.Classes
{
    class DatabaseController
    {
        ...
        {
            return _dataContext.Measurements.ToList();
        }
...
namespace Mallaca
{
    public class DBConnect
    {
        ...
        private List<Usertypes.User> readToUser(MySqlDataReader _reader)
        {
            List<Usertypes.User> users = new List<Usertypes.User>();
        ...
        public List<Usertypes.User>getSpecialistClients(List<Usertypes.User> users)
        {
            ...
            List<Usertypes.User> specialistUsers = specialists.Cast<Usertypes.User>().ToList();
            List<Usertypes.User> finalList = users.Union(specialistUsers).ToList();
            return finalList;
        }

        public List<Usertypes.User> GetAllUsers()
        {
            string usersQuery = String.Format("SELECT user_type, {0}.users.id, Username, name, dateOfBirth, surname, gender, password, length, weight FROM {0}.users LEFT JOIN {0}.client_bmi_info on {0}.users.id = {0}.client_bmi_info.users_id  ", _database);
            OpenConnection();
            _selectCommand = new MySqlCommand(usersQuery, Connection);
            ...
            _reader = _selectCommand.ExecuteReader();
            List<Usertypes.User> users = readToUser(_reader);
        ...
        public bool saveUser(Usertypes.User user)
        {
            OpenConnection();
        ...
        public Usertypes.User getUser(string username)
        {
        ...
        public Usertypes.User getUser(int id)
        {
        ...
        private Usertypes.User getUserQuery(string command){
            OpenConnection();
            _reader = new MySqlCommand(command, Connection).ExecuteReader();
            List<Usertypes.User> user = readToUser(_reader);
            if (user.Count == 0)
                return null;
        ...
        public bool removeUser(Usertypes.User user) 
        { 
            if(user.Id == null)
                return false;
